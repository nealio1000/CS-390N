...
public class TestSingle extends Basic
{
  public static void main(String[] args)
  {
    TestSingle app = new TestSingle();
    app.start();
  }
...
//  private SeparationTracker tracker;
  ...
  public TestSingle()
  {
    ...
    Body body2 = new Body( size, size, size,
              new Triple(26.617187304777367,50,36.6173644767609), Quat.id,
                           Triple.zero, 0, Triple.xAxis,
                           1 );
    list.add( body2 );

    Body body1 = new Body( size, size, size,
           new Triple(41.77394737477645,50,37.80144233624882), Quat.id,
                           Triple.zero, 0, Triple.xAxis,
                           1 );
    list.add( body1 );

    GJKResult result = GJK.gjk( Triple.xAxis, body1, body2 );
    System.out.println( result );

    if( result.v.norm() < .0001 )
      System.out.println("overlap!");

/*
    // two tetras
    Body body1 = new Body( size, 
                 new Triple( 25, 50, 35 ), Quat.id,
                   Triple.zero, 0, Triple.xAxis,
                   1 );
    list.add( body1 );
 
    Body body2 = new Body( size, 
                 new Triple( 41, 50, 37 ), Quat.id,
...
/*
    ...
    Body body = new Body( size, 
     new Triple( randomCoord(), randomCoord(), randomCoord() ), // location
